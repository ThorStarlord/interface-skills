...
The `<ProfileForm>` is the only complex interactive component on this screen. It has its own component spec at [`component-specs/profile-form.md`](component-specs/profile-form.md).
...
## 3. State taxonomy (per [`shared/references/state-taxonomy.md`](shared/references/state-taxonomy.md))
...
| **Submitting** | Save clicked.                                    | Save button shows spinner + label "Saving…"; entire form is `aria-busy="true"` and inputs are disabled. |
| **Success**    | PATCH returns 200.                               | Form returns to Default state with new initial values; toast "Settings updated successfully." |
| **Inline error** | PATCH returns 422 with field errors.            | Affected inputs show `aria-invalid="true"` and an inline error message; Save re-enabled. |
...
Per [`shared/references/accessibility-baseline.md`](shared/references/accessibility-baseline.md), every interactive element is reachable via Tab and shows a visible focus ring.
